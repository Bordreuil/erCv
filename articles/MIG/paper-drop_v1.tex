...
A better observation of behavior of the weld pool or the metal droplet in a Pulsed Gas Metal Arx Welding (PGMAW)
...
of macrodrop or droplets is an important information for the understanding and
...
be done. The intention is to help to simplify future numerical models and enhance monitoring by direct observation.
...
during welding process. Then 
 The library and the algorithms are explained and some results are shown on the macro drop and droplet study.
...
erCv is composed by four processing modules: 
\begin{description}
...
The originality of the library comes from the agregation and complementarity of the different numerical tools.
...
be built between wherever two points of $S$. However, to fulfill
 ...
 is made over $S$ \cite{CGAL}. Each point of $S$ is a vertice 
...
The Delaunay triangulations guarantee a unique solution with minimal
 internal angles for each triangle, which means  minimal length sides to the triangles \cite{GEOMETRICAL}.
...
with $i \in\{1,n\}$, a segment is build between the respective vertices or
 points. At the end of the process, a  set of segments knows as $\alpha$-shape has been created.
  Note if $\sqrt{\alpha} > l_{i} \forall i$, the circle can’t be go inside $\beta$, then $\alpha$-shape
...
Pure Argon shielding lead to a higher desequilibrium at the interface leading to a higher surface tension. Then the angle
