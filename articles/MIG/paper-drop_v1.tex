...
A better observation of behavior of the weld pool or the metal droplet in a PGMAW process
 could help to  welding quality in manufacturing process \cite{LIN}, \cite{WU1}.
 In a PGMAW process, the weld quality is strongly relates with the metal transfer  stability 
 (the droplet deposition). The monitoring of metal transfer process
 will indicate the good wire feed \cite{WANG}. 
The macro drop kinetic monitoring during static PGMAW process could help to better
understand some GMAW related phenomena  \cite{CHO} as free surface 
fluctuations \cite{WANG}. 
...
The monitoring of the shape and size of these welding objects uring
static PGMAW  is 
...
An halogen lamp is use to light the weld process. To guarantee an homogeneous
...
of the welding place. A Phantom V5.0 high speed camera is placed and aligned 
at the other of  the optical path (see figure \ref{schema-montage-experimental-GMAW}). 
...
a $650\ \pm 10\ nm$ band pass filter is place in front the camera lens. 650nm
is around emitting wavelength for the gases. Nevertheless, the raw images remain highly 
...
Two kind of  shielding gas were used \ref{tab::gases}. 
\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|}
\hline
...
\subsection{ The Analyze methods}
...
then  feed the macro drop generated. The macrodrop grows with a geometry that depends on the effects of gravity,
...
The algoritms are tested on a i7 processor from Intel with 1.87Ghz.
...
the second is an indicator of the ease of growth of the macrodrop due to
...
The treatment of each image last around 5ms.
...
Angle and Radius could be measured with the contour. The workpiece surface is detected on the contour so 
that the macrodrop could be isolated. The developped algorithm manage to detect profile under the arc.
The algorithm to extract the macrodrop profile is  a thershold relative to the base material or workpiece.
The result is shown on figure \ref{fig:profile_extract}. Then, calculation of evolution on the profile can pe performed
...
Another important geometrical entity is angle. To approximate it, a simple linear regression is
...
The evolution curves are noisy. Videos investigation show that the macrodrop oscillates during welding. 
Macrodrop oscillations induced left wetting angle oscillations. Depending of the main oscillations during
a period, the local angle oscillation is more or less important. For pure Argon
gas, the equilibrium angle (60 degrees) rapidly reached and stays constant all along the welding.
...
weld pool. These phenomenon near the triple joint are transient. The growth of the macrodrop is a balance mainly between
the energy balance, the mechanical equilibrium (DuprÈ's law and membrane equilibrium). 
The energy balance is coupled to fluid flow in the weld pool. 
The shape of the macrodrop is an important information for
the mechanical equilibrium of the interface and to have information of mechanical acting on it.
...
Nevertheless, the evolution of the basis for the two gases are almost the same (figure \ref{fig::macrodrop}) as well
almost the same overall shape (figure \ref{fig::comp_gazs}). This means that at first order the two evolution
...
(surface computation and position of the center of mass).
...
Once the points of the contour of the droplet, the center of mass could be found as well as the principal axis.
...
\end{figure}
The motions of the center of mass are almost linear and repetitive for the different times.
...
simulation of metal transfert process.
